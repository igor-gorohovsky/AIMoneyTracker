...
def base_currency(create_currency: Callable) -> Callable:
    async def get_base_currency() -> CoroutineType:
        currency_info = CURRENCIES[Service.BASE_CURRENCY]
...
async def test_fetch_currency_rates(
    ...
    sut = Service(db_manager, requester)
...
async def test_update_currency_rates(
    ...
    # Arrange
    mock_data = mock_rates_response(Service.BASE_CURRENCY)
    ...
    # Assert
    base_currency = await get_currency(Service.BASE_CURRENCY)
